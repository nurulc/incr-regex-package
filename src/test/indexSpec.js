...
//
	it("incrRegEx  /abc/", () =>{
		...
		it("incrRegEx /[^\w]123/ - match", () =>{
			var r =  incrRegEx("[^\\w]123");
			expect(r.minChars()).to.equal(convertMask("_123"));
		var res = [ r.match('@') ,r.match('1'),r.match('2') ,r.match('3'),  r.state() ]
			expect(res).to.deep.equal([true,true,true,true,DONE]); 			
		});
		it("incrRegEx for a{3,5} - match", () =>{
		...
		let JSFuncAssign = "([a-zA-Z_?.$][\\w?.$]*)\\.([a-zA-Z_?.$][\\w?.$]*)\\s*(=)\\s*(function)\\s*(\\()(.*?)(\\))";
		it("incrRegEx for Editor highlighting Sound.play = function() ", () =>{
			var r =  incrRegEx(JSFuncAssign);
			var res = [ r.matchStr('Sound.play = function(a,b,c)') , r.state() ];
			expect(res).to.deep.equal([[true, 28, 'Sound.play = function(a,b,c)'],DONE]);
		});
