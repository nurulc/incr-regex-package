...
exports.IREGEX = exports.HOLDER_ZERO_OR_ONE = exports.HOLDER_ANY = exports.HOLDER_ZERO_OR_MORE = undefined;
...
[_rxtree.matchable, _rxtree.boundary, _rxtree.dot, _rxtree.or, _rxtree.zero_or_one, _rxtree.zero_or_more, _rxtree.DONE, _rxtree.FAILED, _rxtree.MORE, _rxtree.MAYBE, _rxtree.makeFSM].map(function (e) {
  if (e === undefined) throw new Error("boundary not defined");
});
...
function makeRxInfo(unit, addElem, merge, optional, mapper) {
  var addOpt = function addOpt(rxNode, prefix) {
    if ((0, _rxtree.zero_or_more)(rxNode)) return addElem(prefix, HOLDER_ZERO_OR_MORE);
    return prefix;
  };
  return getRxInfo;
...
function fixedAt(rxNode) {
  ...
  if (!rxNode || rxNode === _rxtree.DONE) return undefined;
  if ((0, _rxtree.dot)(rxNode)) {
    // this is a node that concat of two regexp /AB/ => dot(A,B) - where A and B are regexp themselves
  ...
  } else if ((0, _rxtree.zero_or_one)(rxNode) || (0, _rxtree.zero_or_more)(rxNode) || (0, _rxtree.boundary)(rxNode)) {
    return undefined;
  }
