...
describe("regexp tests", function () {
  ...
  describe("parser", function () {
    ...
    it("Can parse it own tokenization Rx", function () {
      ...
      (0, _chai.expect)((0, _regexpParser.printExpr)(t.left.left)).to.equal("([.((((\\.u)|((\\.])|((\\.\\)|(((\\?).[)|[^\\]\\[\\\\]))))*).]))"); // match charset [x\[\]\\\u]
      (0, _chai.expect)((0, _regexpParser.printExpr)(t.left.right.left)).to.equal("({.((((\\d.(\\d*)).(,.(\\d.(\\d*))))|((\\d.(\\d*))|(((\\d.(\\d*)).,)|(,.(\\d.(\\d*)))))).}))"); // match {d+,d+} {,d+}  {d+, }
      (0, _chai.expect)((0, _regexpParser.printExpr)(t.left.right.right.left)).to.equal("(\\.(.|(||(+|(*|(?|((|()|(^|($|(d|(D|(s|(S|(b|(B|(w|(W|([|(]|({|(}|\\))))))))))))))))))))))"); // \\ , \. \+ \. \* \? ...
      (0, _chai.expect)((0, _regexpParser.printExpr)(t.left.right.right.right.left)).to.equal("(((.(?.:))|((?.?)|((*.?)|(+.?))))"); // match (?: ??  *? +?
      (0, _chai.expect)((0, _regexpParser.printExpr)(t.left.right.right.right.right.left)).to.equal("(.|(||(+|(*|(?|((|()|(^|$))))))))"); // the standard mets characcters =  +*?.|()^$
    });
  });
