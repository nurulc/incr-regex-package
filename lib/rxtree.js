...
function rxStepArr(lastCh, c, currState) {
  ...
  return matchingStates === undefined ? undefined : [matchingStates, possibleStates];
}
...
function advancedRxMatcherOLD(rxN, str) {
  ...
  return str.substr(1).split('').reduce(function (_ref, c) {
    ...
    var isOk = _ref2[0];
    var lastCh = _ref2[1];
    var currState = _ref2[2];
    var res = _ref2[3];

    var nextState = undefined;
    if (isOk) {
      nextState = rxStepArr(lastCh, c, currState);
...
function advancedRxMatcher(rxN, str) {
  var startNode = ANYCH();startNode.nextNode = rxN; // fake head node, we need to do this because the first 
  // action is to move evrything to next state
  return str.split('').reduce(function (_ref3, c) {
    var _ref4 = _slicedToArray(_ref3, 4);

    var isOk = _ref4[0];
    var lastCh = _ref4[1];
    var currState = _ref4[2];
    var res = _ref4[3];
